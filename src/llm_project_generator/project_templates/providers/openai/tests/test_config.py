...
def test_environment_overrides_default_model(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert Settings(_env_file=None).llm_model == "openai:custom-model"
...
def test_masks_api_key_in_display(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    settings = Settings(_env_file=None)
    assert fake_key not in str(settings.openai_api_key)
...
def test_loads_values_from_env_file(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    env_file.write_text(
        "OPENAI_API_KEY=file-test-key\nLLM_MODEL=openai:file-model\n",
        encoding="utf-8",
    )
    settings = Settings(_env_file=env_file)
    assert settings.openai_api_key.get_secret_value() == "file-test-key"
    assert settings.llm_model == "openai:file-model"
