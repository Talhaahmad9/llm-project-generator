...
def test_generates_anthropic_project_with_anthropic_metadata(tmp_path: Path) -> None:
    destination = tmp_path / "Anthropic_Project"
    generate_project(destination, ANTHROPIC)

    project = tomllib.loads(
        (destination / "pyproject.toml").read_text(encoding="utf-8")
    )
    dependencies = project["project"]["dependencies"]
    assert "pydantic-ai-slim[anthropic]>=2.43.0" in dependencies
    assert not any(
        any(extra in dependency for extra in ("[groq]", "[google]", "[openai]"))
